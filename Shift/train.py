...
def train(opt):
    ...
    while iter < opt.num_iters:
        ...
        for i in range(3):
            out.append(F.pad(state[:, i, b[i]:], p2d[i], "constant", 0))
            
            # if iter == 500:
            #     # print(out[i].shape)
                # cv2.imwrite("{}.png".format(i), out[i][0].cpu().numpy())
            #     # cv2.imwrite("{}.png".format(i), next_state[0, i].cpu().numpy())
        out.append(state[:, 3])
        # cv2.imwrite("{}.png".format(3), next_state[0, 3].cpu().numpy())
        # exit()
        state_p = torch.cat(out)[None]
        ...
        for i in range(3):
            # out.append(F.pad(state[:, i, b[i]:], p2d[i], "constant", 0))
            out_next.append(F.pad(next_state[:, i, b[i]:], p2d[i], "constant", 0))
            ...
            #     # cv2.imwrite("{}.png".format(i), next_state[0, i].cpu().numpy())
        # out.append(state[:, 3])
        out_next.append(next_state[:, 3])
        # cv2.imwrite("{}.png".format(3), next_state[0, 3].cpu().numpy())
        # exit()
        # state_p = torch.cat(out)[None]
        next_state_p = torch.cat(out_next)[None]
        # print(next_state.shape)
        # exit()
