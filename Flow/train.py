...
def train(opt):
    ...
    while iter < opt.num_iters:
        flow = cv2.calcOpticalFlowFarneback(state[0, 3].cpu().numpy(), state[0, 2].cpu().numpy(), None, pyr_scale = 0.5, levels = 3, winsize = 3, iterations = 3, poly_n = 5, poly_sigma = 1.1, flags = 0)
        
        
        # mag, ang = cv2.cartToPolar(flow[..., 0], flow[..., 1])
        # print(np.mean(mag), np.mean(ang))
        # hsv = np.zeros_like(np.repeat(state[0, 1][:, :, None].cpu().numpy(), 3, axis=2))
        # hsv[..., 1] = 255
        # hsv[..., 0] = ang*180/np.pi/2
        # hsv[..., 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX)
        # bgr = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        # ind = np.mean(bgr, axis=2)
        # # print(ind)
        # # bgr[ind<0.1] = [255, 255, 255]
        # cv2.imwrite('flow{}.png'.format(iter), bgr)
        
        flow = torch.from_numpy(flow).cuda().permute(2, 0, 1)[None]

        cv2.imwrite("curr{}.png".format(iter), state[0, 3].cpu().numpy())
        cv2.imwrite("prev{}.png".format(iter), state[0, 2].cpu().numpy())

        # if iter >= 50:
        #     exit()
        # print(flow.shape, np.max(flow), np.min(flow))
        # cv2.imwrite("{}.png".format(3), 255*abs(flow[:, :, 0])/np.max(flow[:, :, 0]))
        # cv2.imwrite("{}.png".format(4), 255*abs(flow[:, :, 1])/np.max(flow[:, :, 1]))
        # exit()
        # print(flow.shape, state.shape)
        state_p = torch.cat((state[:, 3:], flow), dim=1)
        ...
        # if iter == 30:
        #     # print(out[i].shape)
            # cv2.imwrite("{}.png".format(i), out[i][0].cpu().numpy())
        # flow = cv2.calcOpticalFlowFarneback(state[0, 3].cpu().numpy(), state[0, 2].cpu().numpy(), None, pyr_scale = 0.5, levels = 3, winsize = 3, iterations = 3, poly_n = 5, poly_sigma = 1.1, flags = 0)
        flow_next = cv2.calcOpticalFlowFarneback(next_state[0, 3].cpu().numpy(), next_state[0, 2].cpu().numpy(), None, pyr_scale = 0.5, levels = 3, winsize = 3, iterations = 3, poly_n = 5, poly_sigma = 1.1, flags = 0)
        # flow, flow_next = torch.from_numpy(flow).cuda().permute(2, 0, 1)[None], torch.from_numpy(flow_next).cuda().permute(2, 0, 1)[None]
        flow_next = torch.from_numpy(flow_next).cuda().permute(2, 0, 1)[None]
            # cv2.imwrite("{}.png".format(1), state[0, 3].cpu().numpy())
            # cv2.imwrite("{}.png".format(2), state[0, 2].cpu().numpy())
            # print(flow.shape, np.max(flow), np.min(flow))
            # cv2.imwrite("{}.png".format(3), 255*abs(flow[:, :, 0])/np.max(flow[:, :, 0]))
            # cv2.imwrite("{}.png".format(4), 255*abs(flow[:, :, 1])/np.max(flow[:, :, 1]))
            # exit()
        # print(flow.shape, state.shape)
        # state_p = torch.cat((state[:, 3:], flow), dim=1)
        next_state_p = torch.cat((next_state[:, 3:], flow_next), dim=1)#torch.cat(out_next)[None]
